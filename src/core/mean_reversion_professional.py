...
class MeanReversionProfessional:
    ...
    def calculate_keltner_channels(self, high: pd.Series, low: pd.Series, close: pd.Series) -> Tuple[pd.Series, pd.Series, pd.Series]:
        ...
        atr = talib.ATR(high.values, low.values, close.values, timeperiod=self.kc_period)
    ...
    def detect_divergences(self, prices: pd.Series, indicator: pd.Series, lookback: int = 20) -> DivergenceSignal:
        """
    ...
    def find_support_resistance_levels(self, df: pd.DataFrame) -> List[SupportResistanceLevel]:
        ...
        try:
            ...
            for level_name, price in fib_levels.items():
                if level_name in ["23.6", "38.2", "50.0", "61.8"]:
                    levels.append(SupportResistanceLevel(
                        ...
                        type="SUPPORT" if price < df['close'].iloc[-1] else "RESISTANCE",
                        source="FIBONACCI",
                        touches=1
                    ))
            ...
            if len(df) >= 3:
                ...
                levels.extend([
                    SupportResistanceLevel(pivot, 80, "SUPPORT" if pivot < df['close'].iloc[-1] else "RESISTANCE", "PIVOT", 1),
    ...
    def analyze(self, symbol: str, timeframe: str = "1h") -> Optional[MeanReversionSignal]:
        """
        ...
        try:
            ...
            bb_position = (current_price - bb_middle.iloc[-1]) / (bb_upper.iloc[-1] - bb_middle.iloc[-1])
            kc_position = (current_price - kc_middle.iloc[-1]) / (kc_upper.iloc[-1] - kc_middle.iloc[-1])
            ...
            if current_stoch_k < self.stoch_oversold and current_stoch_d < self.stoch_oversold:
                buy_conditions += 1
                notes.append(f"Stochastic oversold: K={current_stoch_k:.1f}, D={current_stoch_d:.1f}")
            ...
            if rsi_divergence.type == DivergenceType.BULLISH:
                ...
                notes.append(f"Divergencia alcista detectada (fuerza: {rsi_divergence.strength:.1f})")
            ...
            if current_stoch_k > self.stoch_overbought and current_stoch_d > self.stoch_overbought:
                sell_conditions += 1
                notes.append(f"Stochastic overbought: K={current_stoch_k:.1f}, D={current_stoch_d:.1f}")
            ...
            if rsi_divergence.type == DivergenceType.BEARISH:
                sell_conditions += 2
                confidence += rsi_divergence.confidence * 0.3
                notes.append(f"Divergencia bajista detectada (fuerza: {rsi_divergence.strength:.1f})")
            ...
            risk_reward_ratio = abs(take_profit_price - current_price) / abs(stop_loss_price - current_price) if stop_loss_price != current_price else 0
            ...
            if len(df) > 20:
                ...
                volume_confirmation = current_volume > avg_volume * self.min_volume_ratio
            ...
            return MeanReversionSignal(
                ...
                nearest_level=nearest_level or SupportResistanceLevel(current_price, 0, "NONE", "NONE", 0),
