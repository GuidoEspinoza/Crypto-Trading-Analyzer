...
class EnhancedRiskManager:
    ...
    def __init__(self, capital_client=None):
        ...
        self.max_drawdown_threshold = (
            self.config.get_max_drawdown_threshold()
        )  # Ya en decimal
        ...
        self.atr_multiplier_range = (
            self.config.get_atr_multiplier_min(),
            self.config.get_atr_multiplier_max(),
        )
        self.trailing_stop_activation = (
            self.config.get_trailing_stop_activation()
        )  # Ya en decimal
        self.breakeven_stop_threshold = (
            self.config.get_breakeven_threshold()
        )  # Ya en decimal
    ...
    def assess_trade_risk(
        ...
        try:
            ...
            print(
                f"🔧 DEBUG: Portfolio value actualizado a: ${self.portfolio_value:.2f}"
            )
            logger.info(
                f"🔧 Portfolio value actualizado a: ${self.portfolio_value:.2f}"
            )
            ...
            profile = TradingProfiles.get_current_profile()
            min_confidence = profile[
                "min_confidence_threshold"
            ]  # Ya está en formato decimal (0.0-1.0)
            is_approved = (
                risk_level not in [RiskLevel.EXTREME, RiskLevel.VERY_HIGH]
                and not max_dd_alert
    ...
    def _configure_dynamic_stop_loss(self, signal: EnhancedSignal) -> DynamicStopLoss:
        ...
        try:
            ...
            if signal.price > 0:
                trailing_distance = (
                    abs(signal.price - initial_stop) / signal.price * 100
                )
            else:
                profile = TradingProfiles.get_current_profile()
                trailing_distance = profile[
    ...
    def _configure_dynamic_take_profit(
        ...
        try:
            ...
            profile = TradingProfiles.get_current_profile()
            tp_increment_pct = profile[
                "tp_increment_base_pct"
            ]  # Valor base desde perfil
            confidence_threshold = (
                risk_config.get_tp_confidence_threshold()
            )  # Desde perfil activo
    ...
    def _calculate_portfolio_risk_metrics(self) -> Dict:
        ...
        try:
            ...
            portfolio_utilization = (
                round(total_position_size / self.portfolio_value, 4)
                if self.portfolio_value > 0
                else 0.0
            )
    ...
    def _calculate_overall_risk_score(
        self, signal: EnhancedSignal, market_risk: Dict, portfolio_metrics: Dict
    ) -> float:
        ...
        try:
            ...
            drawdown_risk = min(
                100, portfolio_metrics.get("current_drawdown", 0) * 500
            )  # Escalar drawdown
        ...
        try:
            ...
            if risk_level == RiskLevel.EXTREME:
                ...
                recommendations.append(
                    "⚠️ Riesgo muy alto - Reducir tamaño de posición 50%"
                )
            ...
            if signal.risk_reward_ratio < 2.0:
                recommendations.append(
                    "Ratio riesgo/beneficio bajo - Buscar mejor entrada"
                )

            # Si no hay recomendaciones específicas
            if not recommendations and risk_level in [
                RiskLevel.VERY_LOW,
                RiskLevel.LOW,
            ]:
    ...
    def _create_default_risk_assessment(
        ...
        fallback_profile = TradingProfiles.get_current_profile()
        default_stop_price = (
            signal.price * 0.95 if signal.signal_type == "BUY" else signal.price * 1.05
        )
        default_tp_price = (
            signal.price * 1.06 if signal.signal_type == "BUY" else signal.price * 0.94
        )
        default_trailing_distance = (
            abs(signal.price - default_stop_price) / signal.price * 100
        )
        return EnhancedRiskAssessment(
        ...
        try:
            if symbol in self.open_positions:
                ...
                if updated_stop:
                    ...
                    logger.info(
                        f"Updated intelligent trailing stop for {symbol}: {updated_stop['new_stop']} - {updated_stop['reason']}"
                    )
    ...
    def _get_current_market_data(self, symbol: str, current_price: float) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "support_distance": abs(current_price - (current_price * 0.98))
                / current_price,
                "resistance_distance": abs((current_price * 1.02) - current_price)
                / current_price,
    ...
    def _update_intelligent_trailing_stop(
        self, position: Dict, current_price: float, market_data: Dict
    ) -> Dict:
        ...
        try:
            ...
            if signal_type == "BUY":
                ...
                if profit_pct >= self.trailing_stop_activation:
                    ...
                    volatility_adj = (
                        1 + (market_data["volatility"] - 0.02) * 10
                    )  # Más espacio si más volátil
                    momentum_adj = (
                        1 - (market_data["momentum"] - 0.5) * 0.2
                    )  # Menos espacio si momentum fuerte

                    adjusted_distance = base_distance * volatility_adj * momentum_adj
                    adjusted_distance = max(
                        0.005, min(0.05, adjusted_distance)
                    )  # Límites 0.5% - 5%
    ...
    def _evaluate_dynamic_position_sizing(self, position: Dict, market_data: Dict):
        ...
        try:
            ...
            if entry_price > 0 and current_price > 0:
                ...
                if (
                    profit_pct > 0.05  # Más del 5% de ganancia
                    and market_data["momentum"] > 0.7  # Momentum fuerte
                    ...
                    if position_size < max_additional:
                        ...
                        position["pyramid_reason"] = (
                            f"Strong momentum ({market_data['momentum']:.2f}) and trend ({market_data['trend_strength']:.2f})"
                        )
                        logger.info(
                            f"Pyramid opportunity identified for {position.get('symbol', 'Unknown')}"
                        )
                ...
                # Si hay alta volatilidad, considerar reducir exposición
                    ...
                    position["reduce_reason"] = (
                        f"High volatility ({market_data['volatility']:.3f})"
                    )
                    logger.warning(
                        f"High volatility warning for {position.get('symbol', 'Unknown')}"
                    )
    ...
    def _update_intelligent_trailing_take_profit(
        self,
        ...
        try:
            ...
            if (
                current_profit_pct < RiskManagerConfig.get_tp_min_percentage()
            ):  # Umbral mínimo configurado (decimal)
                return updated_tp
            ...
            if signal_type == "BUY":
                ...
                if (
                    current_profit_pct >= RiskManagerConfig.get_tp_max_percentage()
                ):  # Umbral máximo configurado (decimal)
                    ...
                    if new_tp > updated_tp.current_tp:
                        ...
                        logger.info(
                            f"TP dinámico actualizado (BUY): {updated_tp.current_tp} "
                ...
                if (
                    ...
                    if new_tp < updated_tp.current_tp:
                        ...
                        logger.info(
                            f"TP dinámico actualizado (SELL): {updated_tp.current_tp} "
                            f"(ganancia: {current_profit_pct*100:.2f}%)"
                        )
        ...
        try:
            ...
            for symbol, position in self.open_positions.items():
                position_risk = {
                    ...
                    "risk_amount": abs(
                        position.get("entry_price", 0) - position.get("current_stop", 0)
                    )
                    * position.get("size", 0),
                    "position_risk_pct": (
                        (position.get("size", 0) / self.portfolio_value) * 100
                        if self.portfolio_value > 0
                        else 0
                    ),
                }
            ...
            if len(self.open_positions) == 0:
                recommendations.append(
                    "Portfolio sin exposición - Buscar oportunidades"
                )
            ...
            return {
                ...
                "exposure_percentage": (
                    round((total_exposure / self.portfolio_value) * 100, 2)
                    if self.portfolio_value > 0
                    else 0
                ),
    ...
    def _calculate_atr_percentile(
        ...
        try:
            ...
            recent_atr_values = (
                df["atr"].dropna().tail(lookback_days * 24)
            )  # Últimos 5 días aprox

            if (
                len(recent_atr_values) < 10
            ):  # Mínimo 10 valores para percentil confiable
                logger.warning(
                    f"Pocos valores de ATR para percentil confiable de {symbol}"
                )
                return {
            ...
            # Calcular percentil
            atr_percentile = (
                (recent_atr_values < current_atr).sum() / len(recent_atr_values) * 100
            )
            ...
            logger.info(
                f"📊 ATR Percentil {symbol}: {atr_percentile:.1f}% | Ajuste: {volatility_adjustment:.1f}x"
            )
