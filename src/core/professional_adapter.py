...
class ProfessionalStrategyAdapter(TradingStrategy):
        ...
        try:
            ...
            try:
                ...
                ema_21 = (
                    ...
                    if not pd.isna(df["ema_21"].iloc[-1])
                    else current_price
                )
                ema_50 = (
                    float(df["ema_50"].iloc[-1])
                    if not pd.isna(df["ema_50"].iloc[-1])
                    else current_price
                )
                atr = (
                    float(df["atr"].iloc[-1])
                ...
                logger.info(
                    f"📊 Indicadores calculados - RSI: {current_rsi:.1f}, EMA21: {ema_21:.2f}, EMA50: {ema_50:.2f}"
                )
                ...
                logger.info(
                    f"✅ Señal profesional generada: {signal_type} ({confidence:.1f}%)"
                )
                ...
                return self._create_hold_signal(
                    symbol, 0.0, f"Error en indicadores: {str(e)}"
                )
    ...
    def _create_hold_signal(
        self, symbol: str, confidence: float, reason: str
    ) -> EnhancedSignal:
...
def create_professional_strategy(
    capital_client: CapitalClient,
) -> ProfessionalStrategyAdapter:
