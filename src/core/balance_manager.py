...
class BalanceManager:
    ...
    def __init__(self, update_interval: int = 30):
        ...
        logger.info(
            f"💰 Balance Manager initialized (update interval: {update_interval}s)"
        )
    ...
    async def _save_balance_to_db(self, balance_info: Dict[str, Any]):
        ...
        logger.debug(
            f"💾 Balance actualizado: ${balance_info.get('available', 0.0):,.2f}"
        )

    async def _periodic_update_task(self):
        """Tarea que actualiza el balance periódicamente"""
        logger.info(
            f"⏰ Iniciando actualizaciones periódicas cada {self.update_interval}s"
        )
        ...
        while self.is_running:
            try:
                ...
                if self.is_running:  # Verificar que aún estamos ejecutándose
                    ...
                    if not success:
                        logger.warning(
                            "⚠️ Falló la actualización de balance, reintentando en el próximo ciclo"
                        )
