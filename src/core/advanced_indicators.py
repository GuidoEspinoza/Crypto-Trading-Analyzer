...
class AdvancedIndicators:
    ...
    def fibonacci_retracement(
        df: pd.DataFrame, lookback: int = None
    ) -> FibonacciLevels:
        """
        ...
        try:
            ...
            if len(recent_data.columns) >= 4:
                swing_high = recent_data.iloc[
                    :, 1
                ].max()  # high es típicamente la columna 1
                swing_low = recent_data.iloc[
                    :, 2
                ].min()  # low es típicamente la columna 2
            else:
        ...
        return FibonacciLevels(
            ...
            level_236=AdvancedIndicators.safe_float(
                swing_high - (diff * FibonacciConfig.RETRACEMENT_LEVELS[0])
            ),  # 0.236
            level_382=AdvancedIndicators.safe_float(
                swing_high - (diff * FibonacciConfig.RETRACEMENT_LEVELS[1])
            ),  # 0.382
            level_500=AdvancedIndicators.safe_float(
                swing_high - (diff * FibonacciConfig.RETRACEMENT_LEVELS[2])
            ),  # 0.500
            level_618=AdvancedIndicators.safe_float(
                swing_high - (diff * FibonacciConfig.RETRACEMENT_LEVELS[3])
            ),  # 0.618
            level_786=AdvancedIndicators.safe_float(
                swing_high - (diff * FibonacciConfig.RETRACEMENT_LEVELS[4])
            ),  # 0.786
    ...
    def ichimoku_cloud(df: pd.DataFrame) -> IchimokuCloud:
        ...
        try:
            ...
            latest_idx = -1
            while pd.isna(ichimoku_data.iloc[latest_idx].iloc[0]) and abs(
                latest_idx
            ) < len(ichimoku_data):
                latest_idx -= 1
            ...
            chikou_span = (
                latest.iloc[4] if len(latest) > 4 else 0
            )  # Generalmente ICS_26
            ...
            high_9 = (
                df["high"]
                .rolling(window=AdvancedIndicatorsConfig.ICHIMOKU_TENKAN_PERIOD)
                .max()
            )
            low_9 = (
                df["low"]
            ...
            high_26 = (
                df["high"]
                .rolling(window=AdvancedIndicatorsConfig.ICHIMOKU_KIJUN_PERIOD)
                .max()
            )
            low_26 = (
                df["low"]
            ...
            # Senkou Span A
            senkou_a = ((tenkan_sen + kijun_sen) / 2).shift(
                AdvancedIndicatorsConfig.ICHIMOKU_SHIFT
            )

            # Senkou Span B (52 períodos)
            high_52 = (
                df["high"]
                .rolling(window=AdvancedIndicatorsConfig.ICHIMOKU_SENKOU_B_PERIOD)
                .max()
            )
            low_52 = (
                df["low"]
            ...
            senkou_b = ((high_52 + low_52) / 2).shift(
                AdvancedIndicatorsConfig.ICHIMOKU_SHIFT
            )
            ...
            senkou_a = (
                senkou_a.iloc[-1]
                if not pd.isna(senkou_a.iloc[-1])
                else senkou_a.dropna().iloc[-1] if not senkou_a.dropna().empty else 0
            )
            senkou_b = (
                senkou_b.iloc[-1]
                if not pd.isna(senkou_b.iloc[-1])
                else senkou_b.dropna().iloc[-1] if not senkou_b.dropna().empty else 0
            )
            chikou_span = (
                chikou_span.iloc[-1]
    ...
    def stochastic_oscillator(
        df: pd.DataFrame, k_period: int = None, d_period: int = None
    ) -> Dict:
        """
        ...
        try:
            ...
            if psar is None or psar.empty:
                ...
                if current_price > (high_20 + low_20) / 2:
                    current_psar = (
                        low_20 * CalculationConfig.APPROXIMATION_FACTORS["close"]
                    )  # Tendencia alcista
                else:
                    current_psar = (
                        high_20 * CalculationConfig.APPROXIMATION_FACTORS["far"]
                    )  # Tendencia bajista
            else:
                # Usar pandas-ta
            ...
            current_psar = AdvancedIndicators.safe_float(
                current_psar,
                current_price * CalculationConfig.APPROXIMATION_FACTORS["very_close"],
            )
    ...
    @classmethod
    def bollinger_bands(
        cls, df: pd.DataFrame, period: int = None, std_dev: float = None
    ) -> Dict:
        """
        ...
        cache_key = cls._get_cache_key(
            df, "bollinger_bands", period=period, std_dev=std_dev
        )
    ...
    def vwap(cls, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if len(df) < 100:
                typical_price = (df["high"] + df["low"] + df["close"]) / 3
                vwap_data = (typical_price * df["volume"]).cumsum() / df[
                    "volume"
                ].cumsum()
            else:
                # Convertir a float64 para evitar warnings de dtype
    ...
    def money_flow_index(df: pd.DataFrame, period: int = None) -> Dict:
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                warnings.simplefilter("ignore", UserWarning)
                mfi = ta.mfi(
                    high_float, low_float, close_float, volume_float, length=period
                )
            ...
            if mfi is None or mfi.empty:
                ...
                money_ratio = positive_flow_sum / (
                    negative_flow_sum + 1e-10
        ...
        try:
            ...
            return {
                ...
                "signal": (
                    "BUY"
                    if current_rsi < 30
                    else "SELL" if current_rsi > 70 else "HOLD"
                ),
    ...
    def enhanced_rsi(cls, df: pd.DataFrame, period: int = None) -> Dict:
        ...
        try:
            ...
            current_rsi_slow = cls.safe_float(
                rsi_slow.iloc[-1] if rsi_slow is not None else current_rsi, current_rsi
            )
        ...
        try:
            ...
            if current_roc > 5.0:
                ...
                if current_roc > 0:
                    ...
                    signal = "SELL"
                    interpretation = (
                        "🔴 ROC anormalmente bajo - Momentum negativo excepcional"
                    )
            else:
                signal = "HOLD"
    ...
    def volume_profile(df: pd.DataFrame, bins: int = None) -> Dict:
        ...
        try:
            ...
            volume_by_price = (
                df.groupby(price_bins, observed=True)["volume"].sum().sort_index()
            )
            ...
            if current_price > vah:
                signal = "SELL"
                interpretation = (
                    "🔴 Precio por encima del Value Area High - Zona de venta"
                )
            elif current_price < val:
                signal = "BUY"
                interpretation = (
                    "🟢 Precio por debajo del Value Area Low - Zona de compra"
                )
            elif (
                abs(current_price - poc_price) / poc_price < 0.01
            ):  # Cerca del POC (1%)
                signal = "HOLD"
                interpretation = (
                    "⚪ Precio cerca del Point of Control - Zona de equilibrio"
                )
    ...
    def support_resistance_levels(
        df: pd.DataFrame, window: int = None, min_touches: int = 2
    ) -> Dict:
        """
        ...
        try:
            ...
            if (
                nearest_resistance
                and abs(current_price - nearest_resistance) / current_price < 0.02
            ):
                signal = "SELL"
                interpretation = "🔴 Precio cerca de resistencia - Posible rechazo"
            elif (
                nearest_support
                and abs(current_price - nearest_support) / current_price < 0.02
            ):
                signal = "BUY"
            ...
            return {
                ...
                "nearest_support": (
                    round(nearest_support, 2) if nearest_support else None
                ),
                "resistance_levels": [
                    round(level, 2) for level in resistance_levels[-5:]
                ],  # Últimos 5
                "support_levels": [
                    round(level, 2) for level in support_levels[-5:]
                ],  # Últimos 5
                "current_price": round(current_price, 2),
                "signal": signal,
                "interpretation": interpretation,
        ...
        try:
            ...
            if len(recent) >= 2:
                ...
                if (
                    ...
                    and latest_close > latest_open  # Vela actual alcista
                    and latest_open < prev_close  # Abre por debajo del cierre anterior
                    and latest_close > prev_open
                ):  # Cierra por encima de la apertura anterior
                    detected_patterns.append(
                        {
                ...
                "patterns": (
                    ...
                    if detected_patterns
                    else [
                        {
                            "name": "Ninguno",
                            "signal": "NEUTRAL",
                            "description": "No se detectaron patrones relevantes",
                        }
                    ]
                ),
            }
                ...
                "patterns": [
                    {
                        "name": "Error",
                        "signal": "NEUTRAL",
                        "description": f"Error detectando patrones: {str(e)}",
                    }
                ],
            }
    ...
    def trend_lines_analysis(df: pd.DataFrame, lookback: int = None) -> Dict:
        ...
        try:
            ...
            if uptrend and current_price > uptrend["current_value"]:
                distance_pct = (
                    (current_price - uptrend["current_value"])
                    / uptrend["current_value"]
                    * 100
                )
            ...
            if downtrend and current_price < downtrend["current_value"]:
                distance_pct = (
                    (downtrend["current_value"] - current_price)
                    / downtrend["current_value"]
                    * 100
                )
                if distance_pct > 0.5:  # Breakout significativo
                    signal = "SELL"
                    interpretation = (
                        f"🔴 Downtrend line breakdown ({distance_pct:.2f}%)"
                    )
                else:
                    interpretation = f"📉 Near downtrend line resistance"

            return {
                "trend_lines": {"uptrend": uptrend, "downtrend": downtrend},
                "signal": signal,
                "interpretation": interpretation,
                "current_price": current_price,
            }
        ...
        try:
            ...
            def detect_ascending_triangle():
                ...
                resistance_touches = sum(
                    1
                    for h in highs[-window:]
                    if abs(h - resistance_level) / resistance_level < 0.01
                )
            ...
            def detect_descending_triangle():
                ...
                support_touches = sum(
                    1
                    for l in lows[-window:]
                    if abs(l - support_level) / support_level < 0.01
                )

                # Verificar si los máximos están bajando
                recent_highs = highs[-window // 2 :]
                if len(recent_highs) >= 3:
                    high_trend = (recent_highs[-1] - recent_highs[0]) / len(
                        recent_highs
                    )
            ...
            def detect_double_top():
                ...
                if len(peaks) >= 2:
                    ...
                    height_diff = (
                        abs(last_two_peaks[1][1] - last_two_peaks[0][1])
                        / last_two_peaks[0][1]
                    )
            ...
            def detect_double_bottom():
                ...
                if len(valleys) >= 2:
                    ...
                    depth_diff = (
                        abs(last_two_valleys[1][1] - last_two_valleys[0][1])
                        / last_two_valleys[0][1]
                    )
