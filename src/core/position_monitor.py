...
class PositionMonitor:
    ...
    def __init__(
        ...
        self.failed_close_attempts = (
            {}
        )  # Dict para contar intentos fallidos por trade_id
        profile = TradingProfiles.get_current_profile()
        self.max_close_attempts = profile.get(
            "max_close_attempts", 3
        )  # Máximo número de intentos antes de marcar como procesado

        # Configuración de monitoreo desde TradingBotConfig
        self.monitor_interval = (
            TradingBotConfig.get_monitoring_interval()
        )  # segundos entre checks
        self.price_cache_duration = CacheConfig.get_ttl_for_operation(
            "price_data"
        )  # segundos de validez del cache
    ...
    def start_monitoring(self):
        ...
        self.monitor_thread = threading.Thread(
            target=self._monitoring_loop, daemon=True, name="PositionMonitor"
        )
    ...
    def _monitoring_loop(self):
        ...
        cleanup_interval = (
            TradingBotConfig.get_cleanup_interval()
        )  # Limpiar cada N ciclos
            ...
            try:
                ...
                for position in active_positions:
                    try:
                        ...
                        logger.warning(
                            f"⚠️ No se pudo obtener precio para {position.symbol}: {e}"
                        )
                        # Continuar con otras posiciones
                        continue
                    except Exception as e:
                        logger.error(
                            f"🚨 Error inesperado obteniendo precio para {position.symbol}: {e}"
                        )
                        continue
                ...
                if market_data:
                    try:
                        updated_count = self.position_manager.update_trailing_stops(
                            market_data
                        )
                        if updated_count > 0:
                    ...
                    try:
                        updated_tp_count = (
                            self.position_manager.update_dynamic_take_profits(
                                market_data
                            )
                        )
                        if updated_tp_count > 0:
                            logger.debug(
                                f"🎯 Updated {updated_tp_count} dynamic take profits"
                            )
                    except Exception as e:
                ...
                for position in active_positions:
                    ...
                    try:
                        ...
                        logger.error(
                            f"❌ Error monitoring position {position.trade_id}: {e}"
                        )
    ...
    def _monitor_position(self, position: PositionInfo):
        ...
        if close_reason:
            ...
            if (
                trade_id not in getattr(self, "_last_log_time", {})
                or time.time() - getattr(self, "_last_log_time", {}).get(trade_id, 0)
                > 60
            ):
                ...
                if position.entry_price is None or position.entry_price == 0:
                    logger.warning(
                        f"⚠️ Position {trade_id} has invalid entry_price: {position.entry_price}"
                    )
                    pnl_pct = 0.0
                else:
                    pnl_pct = (
    ...
    def _execute_position_close(self, status: PositionStatus):
        ...
        try:
            ...
            if result.success:
                ...
                logger.error(
                    f"❌ Failed to close position {status.trade_id}: {result.message}"
                )

        except Exception as e:
            logger.error(
                f"❌ Error executing position close for {status.trade_id}: {e}"
            )
    ...
    def _update_closed_trade(self, status: PositionStatus):
        ...
        try:
            ...
            logger.info(
                f"💰 Final PnL: {status.unrealized_pnl:.2f} ({status.unrealized_pnl_percentage:.2f}%)"
            )
    ...
    def _calculate_trailing_stop(
        ...
        try:
            ...
            trailing_distance = TradingProfiles.get_current_profile()[
    ...
    def _update_trailing_stop(self, status: PositionStatus):
        ...
        try:
            ...
            trailing_stop = self._calculate_trailing_stop(
                position_dict, status.current_price
            )
            if trailing_stop:
                # Actualizar el trailing stop usando PositionManager
                success = self.position_manager.update_position_stop_loss(
                    status.trade_id, trailing_stop
                )
                if success:
                    logger.info(
                        f"📈 Trailing stop actualizado para {status.symbol}: ${trailing_stop:.4f}"
                    )
    ...
    def get_monitoring_status(self) -> Dict:
        ...
        return {
            ...
            "last_update": (
                max(self.last_price_update.values()) if self.last_price_update else None
            ),
            ...
            "trailing_stops_activated": position_stats.get(
