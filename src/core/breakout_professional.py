...
class BreakoutProfessional:
    ...
    def get_market_data(self, symbol: str, timeframe: str, periods: int = 100) -> pd.DataFrame:
        """
    ...
    def calculate_adx(self, high: pd.Series, low: pd.Series, close: pd.Series) -> Tuple[pd.Series, pd.Series, pd.Series]:
        """Calcular ADX, +DI, -DI"""
        adx = talib.ADX(high.values, low.values, close.values, timeperiod=self.adx_period)
        plus_di = talib.PLUS_DI(high.values, low.values, close.values, timeperiod=self.adx_period)
        minus_di = talib.MINUS_DI(high.values, low.values, close.values, timeperiod=self.adx_period)
        
        return pd.Series(adx), pd.Series(plus_di), pd.Series(minus_di)
    
    def find_swing_points(self, df: pd.DataFrame, window: int = 5) -> Tuple[List[Tuple], List[Tuple]]:
    ...
    def detect_consolidation_pattern(self, df: pd.DataFrame) -> ConsolidationInfo:
        ...
        try:
            if len(df) < self.min_consolidation_periods:
                return ConsolidationInfo(ConsolidationPattern.NONE, 0, 0, 0, 0, 0, 0, "STABLE", 0)
            ...
            recent_highs = [h for h in highs if h[0] >= len(df) - 50]  # Últimos 50 períodos
            ...
            start_index = min([h[0] for h in recent_highs] + [l[0] for l in recent_lows])
            end_index = len(df) - 1
            ...
            strength = self._calculate_pattern_strength(recent_highs, recent_lows, duration, range_size)
    ...
    def _classify_pattern(self, highs: List[Tuple], lows: List[Tuple], df: pd.DataFrame) -> ConsolidationPattern:
    ...
    def _analyze_volume_profile(self, df: pd.DataFrame, start_idx: int, end_idx: int) -> str:
    ...
    def _calculate_pattern_strength(self, highs: List[Tuple], lows: List[Tuple], 
                                  duration: int, range_size: float) -> float:
    ...
    def detect_breakout(self, df: pd.DataFrame, consolidation: ConsolidationInfo) -> BreakoutInfo:
        """
        ...
        try:
            ...
            avg_volume = df['volume'].tail(self.volume_lookback).mean() if 'volume' in df.columns else 1
    ...
    def _calculate_momentum_strength(self, df: pd.DataFrame, direction: BreakoutDirection) -> float:
    ...
    def _check_retest(self, df: pd.DataFrame, consolidation: ConsolidationInfo, 
                     direction: BreakoutDirection) -> bool:
        ...
        try:
            ...
            if direction == BreakoutDirection.UPWARD:
                ...
                for _, row in recent_data.iterrows():
                    if abs(row['low'] - retest_level) <= tolerance and row['close'] > retest_level:
                        return True
                ...
                for _, row in recent_data.iterrows():
                    if abs(row['high'] - retest_level) <= tolerance and row['close'] < retest_level:
                        return True
    ...
    def _calculate_false_breakout_risk(self, df: pd.DataFrame, consolidation: ConsolidationInfo) -> float:
        """Calcular el riesgo de falso breakout"""
        try:
            # Buscar falsos breakouts recientes
            lookback_data = df.tail(self.max_false_breakout_lookback + consolidation.duration)
            false_breakouts = 0
            ...
            for i in range(len(lookback_data) - 1):
                ...
                if (current['high'] > resistance and 
                    next_candle['close'] < resistance):
                    false_breakouts += 1
                
                # Falso breakout bajista
                if (current['low'] < support and 
                    next_candle['close'] > support):
                    false_breakouts += 1
    ...
    def calculate_measured_move(self, consolidation: ConsolidationInfo, 
                              breakout: BreakoutInfo) -> float:
    ...
    def determine_trend_alignment(self, df: pd.DataFrame) -> Tuple[bool, TrendDirection]:
    ...
    def analyze(self, symbol: str, timeframe: str = "1h") -> Optional[BreakoutSignal]:
        ...
        try:
            ...
            if consolidation.duration < self.min_consolidation_periods:
                logger.info(f"Consolidación muy corta para {symbol}: {consolidation.duration} períodos")
                return None
            ...
            # 6. Aplicar filtros
            consolidation_duration_ok = consolidation.duration >= self.min_consolidation_periods
            ...
            if breakout.direction != BreakoutDirection.NONE:
                ...
                if breakout.direction == BreakoutDirection.UPWARD:
                    signal_type = "BUY"
                    confidence = 60
                    notes.append(f"Breakout alcista desde {consolidation.resistance_level:.2f}")
                ...
                # Breakout bajista
                    ...
                    signal_type = "SELL"
                    confidence = 60
                    notes.append(f"Breakout bajista desde {consolidation.support_level:.2f}")
                ...
                if volume_confirmation:
                    confidence += 10
                    notes.append(f"Volumen confirmado: {breakout.volume_ratio:.1f}x promedio")
                ...
                if breakout.false_breakout_risk > 50:
                    confidence -= 10
                    notes.append(f"Alto riesgo falso breakout: {breakout.false_breakout_risk:.1f}%")
            
            else:
                # No hay breakout, evaluar si estamos en consolidación
                notes.append(f"Consolidación detectada: {consolidation.pattern.value}")
                notes.append(f"Rango: {consolidation.support_level:.2f} - {consolidation.resistance_level:.2f}")
            ...
            if signal_type != "HOLD" and confidence < self.min_confidence:
                signal_type = "HOLD"
                notes.append(f"Confianza insuficiente: {confidence:.1f}% < {self.min_confidence}%")
            ...
            risk_reward_ratio = (abs(take_profit_price - current_price) / 
                               abs(stop_loss_price - current_price)) if stop_loss_price != current_price else 0
            ...
            return BreakoutSignal(
                ...
                strength="Strong" if confidence > 80 else "Moderate" if confidence > 65 else "Weak",
