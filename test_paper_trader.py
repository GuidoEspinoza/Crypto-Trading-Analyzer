...
def test_paper_trader_initialization():
    ...
    try:
        ...
        assert hasattr(trader, 'balance'), "PaperTrader should have balance"
        assert hasattr(trader, 'positions'), "PaperTrader should have positions"
        assert hasattr(trader, 'trade_history'), "PaperTrader should have trade_history"
        
        # Verificar balance inicial
        if hasattr(trader, 'balance'):
            print(f"💰 Balance inicial: ${trader.balance}")
            assert trader.balance > 0, "Balance should be positive"
